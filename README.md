# 📚 Trabajo Práctico Obligatorio — Programación II (2025)

## 🎯 Objetivo

Implementar nuevos TDA y resolver problemas utilizando exclusivamente las estructuras permitidas por la materia.

---

## 🚫 Restricciones Obligatorias

- No usar Collections Framework.
- No usar ArrayList.
- No usar HashMap.
- No usar LinkedList de Java.
- No usar estructuras ajenas a la cursada.

### Estructuras permitidas

- Arreglos.
- Listas enlazadas vistas en clase.
- Objetos propios.
- Los ocho TDA de la materia.
- P2lib.jar.

---

# 📋 Consignas

## Parte 1 — Implementación y Resolución

| # | Ejercicio | Estado | Ubicación |
|---|------------|---------|-----------|
| 1 | ConjuntoEspecialTDA | ✔️ | `ConjuntoEspecial/src` |
| 2 | ConjuntoMamushkaTDA | ✔️ | `ConjuntoMamushka/src` |
| 3 | MultiPilaTDA | ✔️ | `MultiPila/src` |
| 4 | DiccionarioSimpleModTDA | ✔️ | `DiccionarioSimpleMod/src` |
| 5 | DiccionarioSimpleTDA usando ColaPrioridadTDA | ✔️ | `DiccionarioSimpleConColaPrioridad/src` |
| 6 | % de pares en pila | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 7 | Repetidos en pila | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 8 | Cola sin repetidos | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 9 | Intersección pila-cola | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 10 | Frecuencias en pila | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 11 | Valores únicos de DiccionarioMultiple | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 12 | Suma de impares ABB | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 13 | Hojas pares ABB | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 14 | Vértices puente en grafo | ✔️ | `MetodosExternos/src/Uso/Main.java` |
| 15 | Grado de vértice | ✔️ | `MetodosExternos/src/Uso/Main.java` |

---

## Parte 2 — Árboles

✔️ Resolución teórica paso a paso (con diagramas, rotaciones, factores de balance, splits/merges) en **[`Parte2_Arboles.md`](Parte2_Arboles.md)**.

| # | Tipo | Estado |
|---|------|--------|
| 16 | AVL (20, 29, 21, 12, 7, −20) | ✔️ |
| 17 | AVL (30, 36, 10, 15, 12, −30) | ✔️ |
| 18 | AVL (8, 5, 6, 3, 1, −3) | ✔️ |
| 19 | Árbol B orden 4 | ✔️ |
| 20 | Árbol B orden 5 | ✔️ |

---

## 📦 Entregables por Ejercicio

Cada ejercicio debe incluir:

1. Estrategia.
2. Código Java.
3. Complejidad temporal.
4. Justificación.
5. Casos de prueba.
6. Preservación de estructuras originales.

---

## 👨‍🏫 Defensa

Todos los integrantes deben poder explicar cualquier ejercicio del trabajo.

